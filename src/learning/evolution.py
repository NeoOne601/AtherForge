...
class ExperimentManager:
    ...
    def record_result(self, record: ExperimentRecord) -> None:
        ...
        logger.info("Experiment recorded: %s (Metric: %.4f -> %.4f)", 
                    record.experiment_id, record.baseline_metric, record.new_metric)
    ...
    async def run_experiment(self, mutation: Dict[str, Any], benchmark_fn: Any) -> ExperimentRecord:
        ...
        if new_metric < baseline_metric:
            logger.info("Mutation worsened performance (%.4f < %.4f). Rolling back...", 
                        new_metric, baseline_metric)
            self._git_rollback()
            self.apply_mutation(mutation["gene"], mutation["old"])
            status = "rolled_back"
        else:
            logger.info("Mutation improved performance (%.4f >= %.4f). Keeping change.", 
                        new_metric, baseline_metric)
