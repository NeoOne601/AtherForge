...
async def sync_info(request: Request):
    ...
    return {
        ...
        "pairing_uri": state.sync_manager.discovery.get_pairing_uri(
            state.sync_manager.event_log.get_private_key()
        )
        if hasattr(state.sync_manager.event_log, "get_private_key")
        else "aetherforge://sync?ip=127.0.0.1&port=8765&node=local&key=demo",
        "peers": [
            {"id": p, "ip": ip, "port": port}
            for p, (ip, port) in state.sync_manager.discovery.available_peers.items()
        ]
        if hasattr(state.sync_manager.discovery, "available_peers")
        else [],
