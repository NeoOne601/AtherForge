...
class AutoIndexHandler(FileSystemEventHandler):
    def __init__(self, watch_dir: Path, loop: asyncio.AbstractEventLoop, vector_store: Any, sparse_index: Any):
...
class StreamSyncDirectoryWatcher:
    ...
    async def _boot_sweep(self):
        ...
        print(f"DEBUG: _boot_sweep: found {len(existing_files)} files: {[f.name for f in existing_files]}", flush=True)
