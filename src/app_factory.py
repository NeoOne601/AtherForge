...
async def _nightly_oplora_job(app: FastAPI, force: bool = False):
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if r.status_code == 200:
                    d = r.json()
                    state.system_location = f"{d.get('city')}, {d.get('region')}, {d.get('country')}"
    ...
    sync_event_log = EventLog(db_path=settings.data_dir / "sync_events.db")
    ...
    state.directory_watcher = StreamSyncDirectoryWatcher(live_folder, asyncio.get_running_loop(), state.vector_store, state.sparse_index)
    state.directory_watcher.start()

    # Scheduler
    state.scheduler = AsyncIOScheduler()
    state.scheduler.add_job(_nightly_oplora_job, CronTrigger(hour=3), args=[app], id="nightly_oplora")
    state.scheduler.start()

    state.startup_ms = (time.perf_counter() - t0) * 1000
    logger.info("AetherForge startup complete", duration_ms=round(state.startup_ms, 2))
